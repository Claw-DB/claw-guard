...
fn parse_mask_type(value: &toml::Value) -> GuardResult<MaskType> {
    match value {
        toml::Value::String(kind) => match kind.as_str() {
            ...
            other => Err(GuardError::PolicyParse(format!(
                "unsupported mask type: {other}"
            ))),
        },
        toml::Value::Table(table) => {
            if let Some(max_len) = table.get("truncate") {
                let max_len = max_len.as_integer().ok_or_else(|| {
                    GuardError::PolicyParse("truncate requires integer".to_owned())
                })?;
            ...
            Err(GuardError::PolicyParse(
                "unknown mask type table".to_owned(),
            ))
        }
        _ => Err(GuardError::PolicyParse(
            "invalid mask_type value".to_owned(),
        )),
    }
}
...
fn parse_condition(value: &toml::Value) -> GuardResult<Condition> {
    ...
    if let Some(scope) = table.get("scope_contains") {
        parts.push(Condition::ScopeContains(parse_string(
            scope,
            "scope_contains",
        )?));
    }
    if let Some(risk) = table.get("risk_above") {
        parts.push(Condition::RiskAbove(
            risk.as_float()
                .or_else(|| risk.as_integer().map(|value| value as f64))
                .ok_or_else(|| {
    ...
    if let Some(resource) = table.get("resource_is") {
        parts.push(Condition::ResourceIs(parse_string(
            resource,
            "resource_is",
        )?));
    }
    ...
    F: Fn(Box<Condition>, Box<Condition>) -> Condition,
{
    ...
    let first = iter.next().ok_or_else(|| {
        GuardError::InvalidCondition("logical operator requires at least one child".to_owned())
    })?;
    Ok(iter.fold(first, |acc, condition| {
        constructor(Box::new(acc), Box::new(condition))
    }))
}
...
fn combine_with_and(parts: Vec<Condition>) -> GuardResult<Condition> {
    ...
    Ok(iter.fold(first, |acc, condition| {
        Condition::And(Box::new(acc), Box::new(condition))
    }))
}
...
mod tests {
    ...
    fn condition_role_in() {
        assert!(
            Condition::RoleIn(vec!["analyst".to_owned(), "admin".to_owned()]).matches(&context())
        );
    }
